...
namespace Meowdoku
{
    ...
    public sealed class BoardInputHandler : MonoBehaviour
    {
        ...
        public void Initialize(GameManager manager, BoardView board, EffectsPlayer effects, TutorialController tutorial)
        {
            gameManager = manager;
            boardView = board;
        ...
        public void CommitCat(int row, int column)
        {
            ...
            else if (lostHeart)
            {
                ...
                effectsPlayer.SpawnFloatingLabel(row, column, "-1", ConflictColor);
            }
        }
        ...
        {
            yield return effectsPlayer.PlayCatFoundSequence(gameManager.Board, coord);
            inputLocked = false;
