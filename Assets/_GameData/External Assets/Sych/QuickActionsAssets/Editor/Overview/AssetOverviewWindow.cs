...
namespace Sych.QuickActionsAssets.Editor.Overview
{
    internal sealed class AssetOverviewWindow : EditorWindow
    {
        ...
        [MenuItem("Tools/QuickActions/Overview", false, 5000)]
        public static void ShowWindow()
        {
            Initialize();
            var window = GetWindow<AssetOverviewWindow>(_settings.PluginName);
        ...
        private void OnGUI()
        {
            ...
            GUILayout.Label($"Welcome to {_settings.PluginName}!", _headerLabelStyle);
            GUILayout.Space(VerticalPadding);

            EditorGUILayout.BeginHorizontal();
            GUILayout.Space(HorizontalPadding);
            GUILayout.Label(_settings.PluginDescription, _textStyle);
            GUILayout.Space(HorizontalPadding);
            ...
            EditorGUILayout.LabelField($"IOS bundle: {(_isiOSSupport ? "Included" : "Excluded")}", _isiOSSupport ? _textStyleGreen : _textStyleRed);
            GUILayout.Space(2f);
            EditorGUILayout.LabelField($"Android bundle: {(_isAndroidSupport ? "Included" : "Excluded") }", _isAndroidSupport ? _textStyleGreen : _textStyleRed);
            GUILayout.Space(2f);
            EditorGUILayout.TextArea($"Support: {_settings.SupportEmail}", _labelStyle);
            GUILayout.Space(8f);
            EditorGUILayout.TextArea($"License: {_settings.License}", _textStyleYellow);
            GUILayout.Space(8f);
            ...
            DrawButton("Rate this asset ❤️ (+1 to karma)", () => Application.OpenURL(_settings.CurrentAssetStoreLink));
            DrawButton("Other assets", () => Application.OpenURL(_settings.AllAssetStoreLink));
        ...
        private static void Initialize()
        {
            ...
            _settings = AssetOverviewSettingsProvider.GetSettings();
            _isAndroidSupport = IsTypeExists(_settings.AndroidBundleMainType, _settings.AndroidBundleMainAssembly);
            _isiOSSupport = IsTypeExists(_settings.IosBundleMainType, _settings.IosBundleMainAssembly);
            _isDontShownAgainEnabled = !_settings.IsIntroductionEnabled;
        ...
        {
            _settings.IsIntroductionEnabled = !enable;
            AssetOverviewSettingsProvider.SatSettings(_settings);
        }
        ...
        private class Initializer
        {
            static Initializer()
            {
                if (SessionState.GetBool("is_vs_window_shown", false))
                    return;

                SessionState.SetBool("is_vs_window_shown", true);

                Initialize();
                if(_settings is not { IsIntroductionEnabled: true })
                    return;
