    ...
    public static class NekoSampleLevels
    {
        ...
        private static NekoLevel[] BuildLevels()
        {
            ...
            for (int i = 0; i < TargetLevelCount; i++)
            {
                ...
                int difficultyTier = (i / 3) + 1;
                int transform = variantIndex % TransformCount;
                int colorShift = ((variantIndex / TransformCount) + difficultyTier) % seed.Size;
                string word = WordForLevel(seed, seeds, words, variantIndex);
                string title = TitleForLevel(seed, seeds, i, variantIndex);

                levels.Add(Create(title, word, difficultyTier, LockedCatCountForLevel(i, difficultyTier), seed, transform, colorShift));
            }
        ...
        private static int LockedCatCountForLevel(int levelIndex, int difficultyTier)
        {
            if (levelIndex == 0)
            {
                return 1;
            ...
            return LockedCatCountForTier(difficultyTier);
        }

        private static int LockedCatCountForTier(int difficultyTier)
        {
            if (difficultyTier <= 4)
            {
                return 2;
        ...
        private static NekoLevel Create(string title, string word, int difficultyTier, int lockedCatCount, LevelSeed seed, int transform, int colorShift)
        {
            int size = seed.Size;
            ...
            return new NekoLevel(title, word, regions, solution, LockedCats(solution, lockedCatCount), difficultyTier);
        }
