...
- **There is no in-Unity level *authoring/validation* tooling.** `LevelExporter`, `LevelEditorWindow`, and `NekoLevelValidator` (and `NekoSampleLevels.cs`, the legacy hardcoded generator they supported) have all been deleted — there's no `Meowdoku > Levels > ...` menu for painting/generating/validating a level's content. **The web level editor (`web level editor/`) is still the only way to author or validate a level**: hand-paint or seed-generate it there, confirm it with `npm test`. There IS now one small piece of in-house Editor tooling, purely for *database bookkeeping* (not authoring): `Assets/_GameData/Systems/Scripts/Levels/Editor/LevelManifestSync.cs` keeps `LevelDatabase.asset`'s `levelFiles` order in sync with the shared ordering manifest (see "Level data pipeline" below) — `Meowdoku > Levels > Sync Level Database From Manifest` / `Export Level Order To Manifest`, plus an `AssetPostprocessor` that auto-runs the sync whenever the manifest or any file under `Data/Levels/Default Levels/` changes. If more level-authoring tooling gets rebuilt beyond this, update this section.
...
Levels are JSON files under `Assets/_GameData/Systems/Data/Levels/Default Levels/` (100+ and growing; a sibling `Level Editor Levels/` folder is unreferenced scratch/draft output, not part of the shipped set), indexed in order by `LevelDatabase` (a ScriptableObject holding an ordered `List<TextAsset>`, at `Assets/_GameData/Systems/Scriptables/Level Database/LevelDatabase.asset`). The conversion chain:
...
**`LevelDatabase.levelFiles`'s order is never hand-edited in the Inspector anymore.** A shared ordering manifest, `Assets/_GameData/Systems/Data/Levels/Default Levels/levels-manifest.json` (`{ formatVersion, order: [...] }`, entries are filename stems, not the level's own `id` field — the two are allowed to drift apart once a level's `id` is edited in place), is the single source of truth for level order, editable from either side:

- **Unity side**: `Assets/_GameData/Systems/Scripts/Levels/Editor/LevelManifestSync.cs` reads the manifest and repopulates `LevelDatabase.levelFiles` to match (`Meowdoku > Levels > Sync Level Database From Manifest`), warning about manifest entries with no matching file and about files present but missing from the manifest (would otherwise silently drop out of the shipped game). A companion `AssetPostprocessor` auto-runs this whenever the manifest or any file under `Default Levels/` changes, so pulling web-editor changes and letting Unity reimport is enough — no menu click required in the common case. `Meowdoku > Levels > Export Level Order To Manifest` is the reverse (one-time bootstrap / defensive re-capture): writes the manifest from `LevelDatabase.levelFiles`'s current order.
- **Web editor side**: the Level Library panel (`web level editor/js/app.js`, `// ---------- level library ----------`) opens `Default Levels/` via the File System Access API (Chromium-only — feature-detected, the rest of the editor works without it), lists every level, lets you drag-reorder them (writing the reordered manifest back via "Save Library Order"), and lets you load one into the main editor and save the edit back to its own file in place ("Save to Library" — always writes back to the file it was loaded from, never keyed off a possibly-since-edited `id`, so renaming `id` mid-edit can't fork into a second file).

So: reordering, adding, or editing a level from the web editor and then focusing Unity (letting it reimport) is now the whole workflow — no Inspector dragging.
